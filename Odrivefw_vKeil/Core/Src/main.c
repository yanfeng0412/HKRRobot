...
/* OC4 must be in PWM mode for TIM TRGO �? ADC to work.                         */
/* CubeMX does not support a channel-only PWM without an output pin, so we      */
/* configure it manually here.  Pulse = 1 (triggering fails if Pulse = 0).     */
...
int main(void)
{
  ...
  MX_ADC3_Init();
  /* USER CODE BEGIN 2 */
  /* OC4 must be in PWM mode for TIM TRGO -> ADC triggering to work. */
  OC4_PWM_Override(&htim1);
  OC4_PWM_Override(&htim8);
  /* Enable USART2 RXNE interrupt so the ring-buffer IRQ handler fires. */
  ...
  /* USB IRQ binary semaphore �? start with 0 tokens */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(M0_ENCZ_GPIO_Port, M0_ENCZ_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pins : M0_nCS_Pin M1_nCS_Pin M0_ENCZ_Pin */
  GPIO_InitStruct.Pin = M0_nCS_Pin|M1_nCS_Pin|M0_ENCZ_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

  /*Configure GPIO pin : M1_ENCZ_Pin */
  GPIO_InitStruct.Pin = M1_ENCZ_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(M1_ENCZ_GPIO_Port, &GPIO_InitStruct);
...
void StartDefaultTask(void const * argument)
{
  ...
  MX_USB_DEVICE_Init();
  /* USER CODE BEGIN 5 */
  /* Init communications (USB CDC + UART4 packet protocol) */
  init_communication();

  /* Init motor control (DRV8301, ADC, PWM timers, Hall sensors) */
  init_motor_control();

  /* Motor threads �? high priority, one per axis */
  osThreadDef(task_motor_0, motor_thread, osPriorityRealtime, 0, 512);
  ...
  /* Command-parse thread (USB + UART4 packet framing) */
  ...
  osThreadCreate(osThread(task_packet_timer), NULL);

  /* USART2 heartbeat + command RX (PA2=TX, PA3=RX, 115200 8N1) */
  osThreadDef(task_uart2_test, uart2_test_thread, osPriorityLow, 0, 512);
  osThreadCreate(osThread(task_uart2_test), NULL);

  /* Default task is done �? delete it */
