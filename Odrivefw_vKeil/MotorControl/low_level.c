...
#include "SEGGER_RTT.h" // Add RTT Logging
...
//        .control_mode = CTRL_MODE_POSITION_CONTROL, //see: Motor_control_mode_t
...
#if MOTOR_CONFIG > 0
        .pole_pairs = APP_POLE_PAIRS,             // from app_config.h
        .pos_gain   = APP_POS_GAIN,               // [(counts/s)/counts]
        .vel_gain               = APP_VEL_GAIN,
        .vel_integrator_gain    = APP_VEL_INTEGRATOR_GAIN,
        .vel_limit              = APP_VEL_LIMIT,
        .calibration_current    = APP_CALIBRATION_CURRENT,
        .resistance_calib_max_voltage = APP_RESISTANCE_CALIB_MAXV,
        .dc_bus_brownout_trip_level   = APP_DC_BUS_BROWNOUT_V,
#else
        .pole_pairs = 7,
        .pos_gain   = 20.0f,
        .vel_gain                     = 15.0f / 1000.0f,
        .vel_integrator_gain          = 1.0f,
        .vel_limit                    = 20000.0f,
        .calibration_current          = 10.0f,
        .resistance_calib_max_voltage = 1.0f,
        .dc_bus_brownout_trip_level   = 8.0f,
#endif
        .pos_setpoint = 0.0f,
        .vel_setpoint = 0.0f,
        ...
        .motor_thread = 0,
        .thread_ready = false,
        // Hall mode: wait for explicit $M0,V,x; command before starting control loop.
        // Sensorless: auto-start after calibration.
...
#if MOTOR_CONFIG > 0
            .pm_flux_linkage      = APP_PM_FLUX_LINKAGE,   // from app_config.h
#else
//            .pm_flux_linkage = 1.58e-3f,          // [V / (rad/s)]  { 5.51328895422 / (<pole pairs> * <rpm/v>) }
...
#if MOTOR_CONFIG > 0
            .spin_up_current      = APP_SPIN_UP_CURRENT,
            .spin_up_acceleration = APP_SPIN_UP_ACCELERATION,
            .spin_up_target_vel   = APP_SPIN_UP_TARGET_VEL,
#else
...
#if APP_USE_HALL_SENSOR
        .hall = {
            .hallA_port   = APP_M0_HALL_A_PORT,
            .hallA_pin    = APP_M0_HALL_A_PIN,
            .hallB_port   = APP_M0_HALL_B_PORT,
            .hallB_pin    = APP_M0_HALL_B_PIN,
            .hallC_port   = APP_M0_HALL_C_PORT,
            .hallC_pin    = APP_M0_HALL_C_PIN,
            .motor_dir    = APP_M0_HALL_MOTOR_DIR,
            .phase_offset = APP_M0_HALL_PHASE_OFFSET,
        },
#endif
        .loop_counter = 0,
        .timing_log = { 0 },
    ...
    {    // ---- begin motors[1] ----
        ...
        .error = ERROR_NO_ERROR,
#if MOTOR_CONFIG > 0
        .pole_pairs             = APP_POLE_PAIRS,
        .pos_gain               = APP_POS_GAIN,
        .vel_gain               = APP_VEL_GAIN,
        .vel_integrator_gain    = APP_VEL_INTEGRATOR_GAIN,
        .vel_limit              = APP_VEL_LIMIT,
        .calibration_current    = APP_CALIBRATION_CURRENT,
        ...
        .sensorless = {
            ...
            .V_alpha_beta_memory = { 0.0f, 0.0f },  // [V]
#if MOTOR_CONFIG > 0
            .pm_flux_linkage      = APP_PM_FLUX_LINKAGE,
#else
            .pm_flux_linkage = 1.58e-3f,          // [V / (rad/s)]  { 5.51328895422 / (<pole pairs> * <rpm/v>) }
#endif
            .estimator_good = false,
...
#if APP_USE_HALL_SENSOR
        .hall = {
            .hallA_port   = APP_M1_HALL_A_PORT,
            .hallA_pin    = APP_M1_HALL_A_PIN,
            .hallB_port   = APP_M1_HALL_B_PORT,
            .hallB_pin    = APP_M1_HALL_B_PIN,
            .hallC_port   = APP_M1_HALL_C_PORT,
            .hallC_pin    = APP_M1_HALL_C_PIN,
            .motor_dir    = APP_M1_HALL_MOTOR_DIR,
            .phase_offset = APP_M1_HALL_PHASE_OFFSET,
            // runtime fields zero-initialised by compiler
        },
#endif
        .loop_counter = 0,
        .timing_log = { 0 },
...
float brake_resistance = APP_BRAKE_RESISTANCE;  // [ohm]  set in app_config.h
...
void init_motor_control() {
    ...
    if (motors[0].encoder.use_index || motors[1].encoder.use_index) {
        /* SetupENCIndexGPIO: CubeMX configures EXTI for encoder Z pins at startup */
    }
...
void DRV8301_setup(Motor_t *motor) {
    ...
    // Overcurrent threshold: 30A requested_current_range with 40V/V gain on 750uOhm
    // Vds = 30A * 40V/V * 750e-6 = 0.900V -> use 0p926_V (closest >= 0.9V, fires at ~30.9A)
    local_regs->Ctrl_Reg_1.OC_ADJ_SET = DRV8301_VdsLevel_0p926_V;
...
bool motor_calibration(Motor_t *motor) {
    ...
    // Hall PLL gains (set if Hall mode is active)
    if (motor->rotor_mode == ROTOR_MODE_HALL) {
        float hall_bw = APP_HALL_PLL_BANDWIDTH;
        motor->hall.pll_kp = 2.0f * hall_bw;
        motor->hall.pll_ki = 0.25f * (motor->hall.pll_kp * motor->hall.pll_kp);
        hall_gpio_init(motor);
    }
...
// Decode 3-bit Hall state to a position count [0-5].
// Returns true on valid state; false on illegal state (0b000 or 0b111).
// Sequence matches standard 120° Hall wiring (ported from ODrive devel encoder.cpp).
static bool decode_hall(uint8_t hall_state, int32_t *hall_cnt) {
    switch (hall_state) {
        case 0x01: *hall_cnt = 0; return true;
        case 0x03: *hall_cnt = 1; return true;
        case 0x02: *hall_cnt = 2; return true;
        case 0x06: *hall_cnt = 3; return true;
        case 0x04: *hall_cnt = 4; return true;
        case 0x05: *hall_cnt = 5; return true;
        default:    return false;
    }
}

static float hall_phase_from_pos(Hall_t *hall, float hall_pos) {
    float elec_rad_per_cnt = 2.0f * (float)M_PI / 6.0f;
    float wrapped_pos = hall_pos - 6.0f * floorf(hall_pos / 6.0f);
    float corrected = wrapped_pos - hall->phase_offset;
    return wrap_pm_pi(elec_rad_per_cnt * corrected * (float)hall->motor_dir);
}

uint8_t read_hall_gpio_state(Hall_t *hall) {
    return ((hall->hallA_port->IDR & hall->hallA_pin) ? 1u : 0u) |
           ((hall->hallB_port->IDR & hall->hallB_pin) ? 2u : 0u) |
           ((hall->hallC_port->IDR & hall->hallC_pin) ? 4u : 0u);
}
...
void update_rotor(Motor_t *motor) {
    ...
    case ROTOR_MODE_SENSORLESS:
        ...
        break;
    case ROTOR_MODE_HALL:
        {
            Hall_t *hall = &motor->hall;

            // Sample Hall GPIO pins (reads IDR even when AF is active)
            hall->hall_state = read_hall_gpio_state(hall);

            int32_t hall_cnt;
            if (decode_hall(hall->hall_state, &hall_cnt)) {
                // Delta in range [-2, 3] (CPR=6)
                int32_t delta = hall_cnt - hall->count_in_cpr;
                delta = ((delta % 6) + 6) % 6;  // -> [0,5]
                if (delta > 3) delta -= 6;        // -> [-2,3]

                // Update cumulative and cpr counts
                hall->shadow_count += delta;
                hall->count_in_cpr  = ((hall->shadow_count % 6) + 6) % 6;

                // PLL (cumulative count space)
                hall->pll_pos += current_meas_period * hall->pll_vel;
                float delta_pos = (float)hall->shadow_count - hall->pll_pos;
                hall->pll_pos += current_meas_period * hall->pll_kp * delta_pos;
                hall->pll_vel += current_meas_period * hall->pll_ki * delta_pos;

                // Use interpolated Hall PLL position to avoid 60-degree phase steps.
                hall->phase = hall_phase_from_pos(hall, hall->pll_pos);
            }
        }
        break;
    default:
...
bool using_hall(Motor_t *motor) {
    return motor->rotor_mode == ROTOR_MODE_HALL;
}
...
float get_rotor_phase(Motor_t *motor) {
    ...
    else if (using_hall(motor)) return motor->hall.phase;
    else
        //TODO error handling
        return 0.0f;
}
...
float get_pll_vel(Motor_t *motor) {
    ...
    else if (using_hall(motor)) {
        // Convert Hall counts/s -> mechanical rad/s
        // counts/s * (2pi/6) = electrical rad/s; divide by pole_pairs = mechanical rad/s
        // This matches the unit of vel_setpoint from $M0,V (km/h -> mechanical rad/s)
        return motor->hall.pll_vel * (2.0f * (float)M_PI / 6.0f)
               / (float)motor->pole_pairs
               * (float)motor->hall.motor_dir;
    }
    else
        //TODO error handling
        return 0.0f;
...
// Reconfigure Hall sensor GPIO pins as floating inputs with pull-up.
// Call once during motor_calibration when ROTOR_MODE_HALL is selected.
void hall_gpio_init(Motor_t *motor) {
    Hall_t *hall = &motor->hall;
    ...
    init.Pin = hall->hallA_pin;
    HAL_GPIO_Init(hall->hallA_port, &init);
    init.Pin = hall->hallB_pin;
    HAL_GPIO_Init(hall->hallB_port, &init);
    init.Pin = hall->hallC_pin;
    HAL_GPIO_Init(hall->hallC_port, &init);

    hall->hall_state = read_hall_gpio_state(hall);

    int32_t hall_cnt;
    if (decode_hall(hall->hall_state, &hall_cnt)) {
        hall->count_in_cpr = hall_cnt;
        hall->shadow_count = hall_cnt;
        hall->pll_pos = (float)hall_cnt;
        hall->pll_vel = 0.0f;
        hall->phase = hall_phase_from_pos(hall, hall->pll_pos);
    } else {
        hall->count_in_cpr = 0;
    ...
    while (motor->enable_control) {
        ...
        if (motor->control_mode >= CTRL_MODE_POSITION_CONTROL) {
            if (motor->rotor_mode == ROTOR_MODE_SENSORLESS ||
                motor->rotor_mode == ROTOR_MODE_HALL) {
        ...
        // Hall startup feedforward: guarantee a minimum Iq when the motor is nearly
        // stationary but a non-zero velocity is commanded. This overcomes cogging torque
        // and static friction immediately (instead of waiting 10+ seconds for the
        // integrator to wind up).
        // M0 and M1 use separate thresholds (APP_M0/M1_HALL_STARTUP_CURRENT) because
        // phase_offset calibration quality may differ between channels – a sub-optimal
        // offset reduces effective torque as T_eff = Iq × cos(θ_error), so the loaded
        // startup current for M1 can be raised independently without touching FOC tuning.
#if APP_USE_HALL_SENSOR
        if (motor->rotor_mode == ROTOR_MODE_HALL &&
            motor->control_mode >= CTRL_MODE_VELOCITY_CONTROL &&
            motor->hall.hall_state >= 1 && motor->hall.hall_state <= 6) {
            float pll_spd = fabsf(get_pll_vel(motor));
            if (fabsf(vel_des) > APP_HALL_STARTUP_VEL_THRESH &&
                pll_spd < APP_HALL_STARTUP_VEL_THRESH) {
                static const float startup_I[2] = {
                    APP_M0_HALL_STARTUP_CURRENT,
                    APP_M1_HALL_STARTUP_CURRENT
                };
                int midx = (motor == &motors[0]) ? 0 : 1;
                float min_I = startup_I[midx];
                if (vel_des > 0.0f && Iq <  min_I) Iq =  min_I;
                if (vel_des < 0.0f && Iq > -min_I) Iq = -min_I;
            }
        }
#endif
        ...
        // Hall-mode stall watchdog: if velocity setpoint is non-trivial but the
        // motor hasn't moved for 3 s, shut down to prevent integrator-driven runaway.
        if (motor->rotor_mode == ROTOR_MODE_HALL &&
            motor->control_mode >= CTRL_MODE_VELOCITY_CONTROL) {
            static uint32_t hall_stall_cnt[2] = {0, 0};
            int midx = (motor == &motors[0]) ? 0 : 1;
            float pll_vel_abs = fabsf(get_pll_vel(motor));
            if (fabsf(vel_des) > 1.0f && pll_vel_abs < 0.5f) {
                hall_stall_cnt[midx]++;
                // current_meas_hz ≈ 8200 Hz → 3 s ≈ 24600 counts
        ...
        // Active-brake halt timeout: auto-disable after the braking window expires
        // (set by H command). This implements "decelerate then cut power" semantics:
        //   H command → vel_setpoint=0, halt_timeout_ticks=N, enable_control stays true
        //   Velocity controller applies braking Iq each cycle.
        //   When either (a) motor stops (|vel| < 2 rad/s) or (b) timeout expires,
        //   enable_control is cleared → loop exits → PWM disabled.
        //   If a V command arrives before timeout, it resets halt_timeout_ticks=-1.
        if (motor->halt_timeout_ticks > 0) {
            --motor->halt_timeout_ticks;
            bool stopped = (fabsf(get_pll_vel(motor)) < 2.0f);
            if (motor->halt_timeout_ticks == 0 || stopped) {
                if (fabsf(motor->vel_setpoint) < 1.0f) {  // still in halt (no new V cmd)
                    motor->enable_control = false;
                }
                motor->halt_timeout_ticks = -1;
            }
        }
        if (!motor->enable_control) break;  // exit after halt auto-disable
        ...
        if (++log_counter >= 2000) {
            log_counter = 0;
            int motor_idx = (motor == &motors[0]) ? 0 : 1;
            SEGGER_RTT_printf(0, "M%d|%s|E:%d|Mod:%d|Iq:%.2f/%.2f|Ang:%.2f\n", 
                motor_idx,
                "Loop", 
                motor->error, 
                motor->control_mode, 
                (double)motor->current_control.Iq_measured, 
                (double)motor->current_control.Iq_setpoint,
                (double)get_rotor_phase(motor));
        }
...
void motor_thread(void const *argument) {
    ...
    for (;;) {
        if (motor->do_calibration == false) {
            ...
            // Drain any stale M_SIGNAL_PH_CURRENT_MEAS before calibration.
            // At re-calibration time the signal bit is already set from ADC IRQs
            // that fired while the thread was sleeping.  The first osSignalWait()
            // in measure_phase_resistance() would then return instantly at a
            // random TIM8 phase → check_timing reads CNT in the downcount half
            // → timing = 1.5-2× period > control_deadline → ERROR_PHASE_RESISTANCE_TIMING.
        ...
        if (motor->calibration_ok && motor->enable_control) {
            ...
            // Drain any stale M_SIGNAL_PH_CURRENT_MEAS signals that accumulated
            // while the motor was halted (ADC IRQ keeps firing even with PWM off).
            // Without this the first osSignalWait() returns immediately at a
            // random timer phase → check_timing reads CNT in the downcount half
            // → timing > control_deadline → ERROR_FOC_TIMING on the very first cycle.
            // osSignalClear is not implemented in this CMSIS wrapper; use a
            // non-blocking osSignalWait(timeout=0) to consume the pending bit instead.
            osSignalWait(M_SIGNAL_PH_CURRENT_MEAS, 0);

            // Reset velocity integrator so a wound-up value from the previous
            // run doesn't cause immediate current saturation on restart.
            motor->vel_integrator_current = 0.0f;
        ...
        queue_voltage_timings(motor, 0.0f, 0.0f);
        osDelay(10);  // 10 ms: fast enough to pick up V command re-enable within 10 ms
    }
