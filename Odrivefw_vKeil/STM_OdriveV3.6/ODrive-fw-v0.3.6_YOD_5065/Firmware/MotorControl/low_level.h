...
#define PH_CURRENT_MEAS_TIMEOUT 2 // [ms]
    ...
    typedef enum {
        ...
        ERROR_MOTOR_STALL,//24  Hall motor stalled under velocity control
    } Error_t;
...
// highest level of control, to allow "<" style comparisons.
    ...
    typedef enum {
        ...
        ROTOR_MODE_HALL,                          //Use Hall sensors for rotor phase sensing
    } Rotor_mode_t;
    ...
    // Hall sensor state (CPR = 6 per electrical revolution)
    typedef struct {
        // Pin configuration — set once in motors[] initializer
        ...
        uint16_t      hallC_pin;
        int32_t       motor_dir;     // 1 or -1
        float         phase_offset;  // electrical sector offset [count], fractional values allowed
        // Runtime state
        uint8_t       hall_state;    // 3-bit GPIO reading
        int32_t       shadow_count;  // cumulative count (for PLL)
        int32_t       count_in_cpr;  // count mod 6  [0-5]
        float         phase;         // [rad] electrical phase
        float         pll_pos;       // PLL position estimate [count]
        float         pll_vel;       // PLL velocity estimate [count/s]
        float         pll_kp;        // [1/s]
        float         pll_ki;        // [1/s^2]
    } Hall_t;
...
#define TIMING_LOG_SIZE 16
    typedef struct {
        ...
        bool calibration_ok;
        int32_t halt_timeout_ticks; // >0 = active-brake countdown; -1 = idle
