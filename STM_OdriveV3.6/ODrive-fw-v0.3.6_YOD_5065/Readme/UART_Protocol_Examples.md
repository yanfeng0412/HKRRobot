...
| 现象 | 可能原因 | 处理 |
|------|---------|------|
| error=24 | Hall 电机失速保护 (stall 3 s) | 检查 `APP_Mx_HALL_MOTOR_DIR` / `PHASE_OFFSET`；电机方向反转时将 motor_dir 取反 |
| error=23 | 母线欠压 | 检查电源电压 ≥ 8V |
| error=20 | DRV8301 驱动器故障 | 检查供电/散热，复位电源 |
| error=9  | ADC 故障 | 检查电流传感器供电 |
| error=1  | 校准时序超时 (TIM8 下计数) | 正常情况下已由代码修复 (signal drain)；若仍出现请检查 TIM8 初始化 |
| 电机嗡嗡不转 | 估算器未锁相 | 适当增大 `APP_SPIN_UP_CURRENT` 或降低 `APP_SPIN_UP_ACCELERATION` |
| M1 越转越快 | `APP_M1_HALL_MOTOR_DIR` 方向反 | 将 `APP_M1_HALL_MOTOR_DIR` 改为 -1 重新编译 |
| M1 来回振荡 | `APP_M1_HALL_PHASE_OFFSET` 错误 | 依次尝试 0.5/1.5/2.5/3.5/4.5/5.5，以电机平稳运行为准 |
